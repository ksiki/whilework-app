...
- Ставь is_vacancy = true ТОЛЬКО при одновременном выполнении двух условий:
  ...
  2. В тексте есть ХОТЯ БЫ ОДИН ЯВНЫЙ контакт для отклика (ссылка, @юзернейм, почта, телефон). ВАЖНО: фразы «пишите в ЛС», «пишите в DM» или «стучитесь в личку» БЕЗ указания конкретного контакта — НЕ СЧИТАЮТСЯ контактом.
...
- Ставь is_vacancy = false, если это:
  ...
  5. Вакансия БЕЗ контактов (вообще нет никаких контактных данных для отклика, либо написано только «пишите в ЛС/DM», но сам юзернейм или ссылка не указаны).
...
3. ЛОКАЦИИ (Locations) и РЕЛОКАЦИЯ (Relocation):
- Страны и города пиши строго на русском языке ("Мск" -> "Москва", "USA" -> "США").
- В релокацию пишется ТОЛЬКО страна (Если релокация вообще указана)
...
- Если в тексте написано «пишите в ЛС» или «DM», но самого юзернейма нет, СТРОГО возвращай пустой массив [] в поле contacts. Не пытайся записывать слова "ЛС" или "DM" как контакт.
- В поле `details` записывай сырой контакт именно в том виде, в котором он указан в тексте... (твой текущий текст)
