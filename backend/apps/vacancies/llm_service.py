...
- Ставь is_vacancy = true ТОЛЬКО при одновременном выполнении двух условий:
  ...
  2. В тексте есть ХОТЯ БЫ ОДИН валидный контакт для отклика: Telegram, Email, Discord, либо ссылка на Google-форму или Яндекс-форму.
- Ставь is_vacancy = false, если это:
  ...
  5. Вакансия БЕЗ контактов (или если указаны только недопустимые ссылки, например, на внутренние сайты или job-борды).
В поле `reasoning` обязательно обоснуй свое решение, явно указав, какой именно валидный контакт был найден, либо констатировав его отсутствие.
...
8. КОНТАКТЫ (Contacts):
- Извлекай только валидные контакты для связи (Telegram, почта, Discord и формы).
- ФОРМЫ: Допускаются ТОЛЬКО прямые ссылки на Google Forms (docs.google.com/forms/...) или Yandex Forms (forms.yandex.ru/...). Любые другие формы, карьерные сайты (hh.ru, habr.com) и внутренние домены компаний СТРОГО ИГНОРИРУЙ.
- В поле `details` записывай СТРОГО чистый контакт: юзернейм (например, "@username"), ссылку (например, "t.me/username"), email ("hr@company.com") или полную ссылку на форму.
